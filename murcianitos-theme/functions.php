...
// Asignar automáticamente la página de términos a WooCommerce
function murcianitos_set_wc_terms_page() {
    $page = get_page_by_path('aviso-legal');
    if ($page && !get_option('woocommerce_terms_page_id')) {
        update_option('woocommerce_terms_page_id', $page->ID);
    }
}
add_action('init', 'murcianitos_set_wc_terms_page');

// ─── 2. Enqueue de estilos y scripts ───
function murcianitos_enqueue_assets() {
    $theme_uri = get_template_directory_uri();
    // Cache buster: usar la fecha de modificación del archivo
    $style_path = get_stylesheet_directory() . '/style.css';
    $theme_ver = file_exists($style_path) ? filemtime($style_path) : wp_get_theme()->get('Version');
...
function murcianitos_create_required_pages() {
    $pages = array(
        ...
        'alergenos' => 'Tabla de Alérgenos',
        'blog-bando-huerta' => 'El Bando de la Huerta',
        'blog-chato-murciano' => 'Chato Murciano',
        'blog-ediciones-limitadas' => 'Ediciones Limitadas',
        'blog-huevo-duro' => '¿Por qué huevo duro?',
        'blog-manteca-iberica' => 'La manteca ibérica',
        'blog-maridaje' => 'Maridaje perfecto',
        'blog-obrador-dentro' => 'El obrador por dentro',
        'blog-origen-pastel' => 'El origen del pastel de carne',
        'blog-secreto-hojaldre' => 'El secreto de nuestro hojaldre'
    );
